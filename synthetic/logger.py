...
class Logger(object):
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            for i in range(result.size(1)-2):
                r = (result[argopt, i+2] - result[argopt, 0]) / result[argopt, 0] * 100
                print(f'Final OOD Test: {result[argopt, i+2]:.2f} {r:.2f}%')
            ...
            for i in range(ood_size):
                ...
                r = (best_result[:, i+4+ood_size] - best_result[:, ood_size+2]) / best_result[:, ood_size+2] * 100
                print(f'   Final OOD Test Ratio: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 4+ood_size:4+2*ood_size].mean(dim=-1)
            print(f'   Final All OOD Test: {r.mean():.2f} ± {r.std():.2f}')
            r = (best_result[:, 4+ood_size:4+2*ood_size].mean(dim=-1) - best_result[:, ood_size + 2]) / best_result[:, ood_size + 2] * 100
            print(f'   Final All Test Ratio: {r.mean():.2f} ± {r.std():.2f}')
            return best_result[:, -ood_size-2:]

import os
def save_result(args, results):
    if not os.path.exists(f'results/{args.dataset}-{args.syn_type}'):
        os.makedirs(f'results/{args.dataset}-{args.syn_type}')
    filename = f'results/{args.dataset}-{args.syn_type}/result.csv'
    print(f"Saving results to {filename}")
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(
            f"{args.method} " + f"{args.encoder}: " + f"{args.diffusivity}: " + f"{args.solver} " + f"{args.num_layers} " + "\n")
        m = ""
        for i in range(results.size(1)):
            r = (results[:, i] - results[:, 0]) / results[:, 0]
            m += f"{r.mean():.2f} $\pm$ {r.std():.2f} "
        r = (results[:, 2:].mean(dim=-1) - results[:, 0]) / results[:, 0]
