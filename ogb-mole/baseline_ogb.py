...
def main():
    ...
    return train_curve[best_val_epoch], valid_curve[best_val_epoch], test_curve[best_val_epoch]
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        te_.append(te)
    r = np.array(tr_) * 100
    print(f'All Train: {r.mean():.4f} ± {r.std():.4f}')
    r = np.array(val_) * 100
    print(f'All Valid: {r.mean():.4f} ± {r.std():.4f}')
    r = np.array(te_) * 100
    print(f'All Test: {r.mean():.4f} ± {r.std():.4f}')
