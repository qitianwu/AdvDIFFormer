...
class AdvDIFFormer(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=1, num_heads=1, solver='series',
                    K_order=3, beta=0.5, theta=0., dropout=0.5, use_bn=True, use_residual=True, task='node-reg'):
        ...
        self.predictor = nn.Linear(2*hidden_channels, out_channels)
    ...
    def forward(self, x, edge_index, batch=None, edge_weight=None, block_wise=False, edge_index_sampled=None):
    ...
    def loss_compute(self, d, criterion, args):
        if self.task == 'link-pre':
            logits = self.forward(d.x, d.edge_index, d.batch, block_wise=args.use_block, edge_index_sampled=d.edge_index_sampled)
        else:
